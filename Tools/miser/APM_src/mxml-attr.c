              ...
              char        *value)	/* I - Attribute value */
{
  ...
  */
  ...
  if (node->value.element.num_attrs == 0)
    attr = (mxml_attr_t*)os_malloc(sizeof(mxml_attr_t));
  else {
    //attr = os_realloc(node->value.element.attrs,
    //               (node->value.element.num_attrs + 1) * sizeof(mxml_attr_t));
    attr = (mxml_attr_t*)os_malloc((node->value.element.num_attrs + 1) * sizeof(mxml_attr_t));
    if (attr) {
