  ...
  */
  ...
  switch (node->type)
  {
    case MXML_ELEMENT :
	  ...
	  os_free(node->value.element.name);
	...
	if (node->value.element.num_attrs)
	{
	  for (i = 0; i < node->value.element.num_attrs; i ++)
	  {
	    if (node->value.element.attrs[i].name)
	      os_free(node->value.element.attrs[i].name);
	    if (node->value.element.attrs[i].value)
	      os_free(node->value.element.attrs[i].value);
	  }

          os_free(node->value.element.attrs);
	}
        break;
    case MXML_INTEGER :
       /* Nothing to do */
    ...
    case MXML_TEXT :
	  ...
	  os_free(node->value.text.string);
        break;
    case MXML_CUSTOM :
 ...
 *
 * Does not os_free memory used by the node - use mxmlDelete() for that.
         ...
         mxml_type_t type)		/* I - Node type */
{
  ...
  */
  ...
  if ((node = (mxml_node_t*)os_malloc(sizeof(mxml_node_t))) == NULL)
  {
#if DEBUG > 1
