...
static inline int	mxml_isspace_only(int ch)
			{
			  return (ch == ' ');
			}
 ...
 * using the os_free() function when you are done with it.  NULL is returned
	      ...
	      int  *bufsize)		/* IO - Current buffer size */
{
  ...
  if (*bufptr >= (*buffer + *bufsize - 4))
  {
    ...
    */
    ...
    //if ((newbuffer = os_realloc(*buffer, *bufsize)) == NULL)
    if ((newbuffer = os_malloc(*bufsize)) == NULL)
    {
      os_free(*buffer);
...
//         (mxml_isspace(ch) && type != MXML_OPAQUE && type != MXML_CUSTOM)) &&
         (mxml_isspace(ch) && type != MXML_OPAQUE && type != MXML_CUSTOM && !(mxml_isspace_only(ch) && type == MXML_TEXT))) &&
        bufptr > buffer)
    {
     /*
	...
	*/
    ...
    else if (type == MXML_OPAQUE || type == MXML_CUSTOM || !mxml_isspace(ch) || (mxml_isspace_only(ch) && (type == MXML_TEXT)))
    {
     /*
