...
void EMVDebug(void)
{
  ...
  memcpy(buf, "TVR: ", 5);
  ptag = TagSeek(gGDS->s_TagList, 0x95);    // TVR
  if (ptag != NULL)
    split(&buf[5], ptag->pbLoc, 5);
  DispLineMW(buf, MW_LINE3, MW_SPFONT|MW_CLREOL);

  memset(buf, 0, sizeof(buf));
  memcpy(buf, "TSI: ", 5);
  ptag = TagSeek(gGDS->s_TagList, 0x9B);    // TSI
  if (ptag != NULL)
    split(&buf[5], ptag->pbLoc, 2);
  DispLineMW(buf, MW_LINE4, MW_SPFONT|MW_CLREOL);
  APM_WaitKey(9000,0);
}
//*****************************************************************************
//  Function        : EMVProcess
