...
//*****************************************************************************
//  Function        : multicurrencyFile
//  Description     : read file data from param file.
//  Input           : N/A
//  Return          : N/A
//  Note            : N/A
//  Globals Changed : N/A
//*****************************************************************************
BOOLEAN multicurrencyFile(BYTE aFileNo,BYTE *binacc,char comm)
{
...
//DispLineMW(binacc, MW_LINE5, MW_SPFONT);
  
i=0;

while(true){

multi=memcmp ( binacc,&ptr[i],6);
...
//  Globals Changed : N/A
//*****************************************************************************
BOOLEAN blacklistFile(BYTE aFileNo,BYTE *binacc)
{
  T_FILE file_info, *pInfo;
  int    fd, fsize, rlen;
  BYTE   gif_file[64], *ptr;
...
if (blacklistFile(bytes[0],&tmpbuf[1])== TRUE) {
        }
        else {
			 DispLineMW("Black List", MW_LINE4, MW_CLRDISP|MW_CENTER|MW_SMFONT);
            	while(true){
   keyin = APM_GetKeyin();
  if (keyin == MWKEY_ENTER)
    break;
		...
		if(INPUT.currency ==1){
// in dollar and other currency
		   if (multicurrencyFile(bytes[1],&tmpbuf[1],'1')== TRUE) {
...
if(INPUT.currency==2){
// in EURO
		   if (multicurrencyFile(bytes[1],&tmpbuf[1],'2')== TRUE) {
			    DispLineMW("Not Permit", MW_LINE4, MW_CLRDISP|MW_CENTER|MW_SMFONT);
            	while(true){
   keyin = APM_GetKeyin();
  if (keyin == MWKEY_ENTER)
