...
// Constant
//-----------------------------------------------------------------------------
static const char KLogoTmpFileName[] = {"LogoTmp"};
static const char KLogoFileName[] = {"Logo"};
 ...
 DWORD keyin;
  ...
  if (GIFLoadMem(ptr, fsize)) {
    ...
    //  }
	 ...
	 keyin = APM_WaitKey(1000, WAIT_ICC_INSERT);
  if (keyin == MWKEY_LEFT){
	  fuctiongo=1;
...
void ParamTest(void)
{
  ...
  os_config_read(K_CF_LcdTimeout, &bl_timeout);
  os_disp_bl_control(0xFF);
  //os_disp_putc(K_PushCursor);

  while (1) {
    DispLineMW("DISPLAY PARAM NO.:", MW_LINE4, MW_CLRDISP|MW_SMFONT);
    memset(kbdbuf, 0, sizeof(kbdbuf));
    if (!APM_GetKbd(NUMERIC_INPUT+ECHO+RIGHT_JUST+MW_LINE7, IMIN(1)+IMAX(1), kbdbuf))
      return;

    if (DispParamFile(kbdbuf[1]-'0') == TRUE) {
      DispClrBelowMW(MW_LINE3);
      DispLineMW("Display Done!", MW_LINE4, MW_CLRDISP|MW_CENTER|MW_SMFONT);
      APM_WaitKey(500, 0);
    }
    else {
      DispClrBelowMW(MW_LINE3);
      DispLineMW("Display Err!", MW_LINE4, MW_CLRDISP|MW_CENTER|MW_SMFONT);
      APM_WaitKey(500, 0);
    }
  }
  //os_disp_putc(K_PopCursor);
  os_disp_bl_control(bl_timeout);
}
//*****************************************************************************
...
static DWORD LptOpen(void *aCfg)
{
  ...
  if (gLptHandle >= 0) {
    if (IOCtlMW(gLptHandle, IO_TMLPT_CFG, aCfg) == -1)
      return FALSE;
    StatMW(gLptHandle, MW_TMLPT_RESET, NULL);
    return TRUE;
  }
  return FALSE;
}
...
{
  DWORD status = StatMW(gLptHandle, MW_TMLPT_NORSTATUS, NULL);
  while (aLen--) {
    status = LptPutC(*aDat);
    if ((status & 0xFF) != MW_TMLPT_SOK)
      break;
...
//  Globals Changed : N/A
//*****************************************************************************
DWORD LptPutG(const BYTE *aBuf, DWORD aLen)
{
  DWORD i, dw;
  DWORD status=0;
  
  for (i=0; i<aLen; i+=sizeof(DWORD)) { 
    memcpy(&dw, aBuf, sizeof(DWORD));
    while (os_lpt_putdw(dw) != dw) {
      status = StatMW(gLptHandle, MW_TMLPT_NORSTATUS, NULL);
      if ((status & 0xFF) != MW_TMLPT_SOK)
        return status;
    }
...
static DWORD PrintLogo(DWORD aWidth, DWORD aHeight, const BYTE *aLogoPtr)
{
  ...
  status = LptPutG(aLogoPtr, logobuf_size);
...
void ParamLptTest(void)
{
  ...
  while (1) {
    DispLineMW("PRINT PARAM NO.:", MW_LINE4, MW_CLRDISP|MW_SMFONT);
    memcpy(kbdbuf, "\x01""5", 2);
    if (!APM_GetKbd(NUMERIC_INPUT+ECHO+RIGHT_JUST+MW_LINE7, IMIN(1)+IMAX(1), kbdbuf))
      return;

    if (LptParamFile(kbdbuf[1]-'0') == TRUE) {
      DispClrBelowMW(MW_LINE3);
      DispLineMW("Lpt Done!", MW_LINE4, MW_CLRDISP|MW_CENTER|MW_SMFONT);
      APM_WaitKey(500, 0);
    }
    else {
      DispClrBelowMW(MW_LINE3);
      DispLineMW("Lpt Err!", MW_LINE4, MW_CLRDISP|MW_CENTER|MW_SMFONT);
      APM_WaitKey(500, 0);
    }
  }
  os_disp_bl_control(bl_timeout);
