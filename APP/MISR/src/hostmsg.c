...
//    get_mem(&RSP_DATA.s_dtg.b_hour, 12);
  ...
  if (bitmap[2] & 0x04) {
       memset(field, 0, sizeof(field));
      print_field(&field[0],12,22);
   // inc_pptr(12);
  }
   /* 23. Pan Sequence Number */
  if (bitmap[2] & 0x02) {
       memset(field, 0, sizeof(field));
      print_field(&field[0],3,23);
   // inc_pptr(3);
  }
  /* 24. bypass netword id */
  if (bitmap[2] & 0x01) {
       memset(field, 0, sizeof(field));
      print_field(&field[0],3,24);
   // inc_pptr(3);
  }
 ...
 /* 42.Card acceptor identification */
  ...
  if (bitmap[5] & 0x01) {
      ...
      var_i = bcd2bin(get_word());
      memset(RSP_DATA.sb_pan, 0xFF, 10);
      print_field(&field[0],var_i,48);
  }

   /* 49.	Transaction currency code */
  if (bitmap[6] & 0x80) {
       memset(field, 0, sizeof(field));
          print_field(&field[0],3,49);
	// inc_pptr(3);
  }
