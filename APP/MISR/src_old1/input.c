...
//      Globals Variables
//-----------------------------------------------------------------------------
cur_info_t gCurInfo[MAX_CUR_SUPPORT] =
{
  {"EGP", "818"},
  {"USD", "840"},
  {"EUR", "978"},
 ...
 *  Description     : Check currency selected with acquirer table
 *  Input           : type: 0 - input.currency
 *                          1 - tx_data.currency
 *                    stop: TRUE  - show msg & stop if error
 *                          FALSE - not stop even on error
 ...
 ******************************************************************************
 */
BOOLEAN CheckCurrency(BYTE type, BOOLEAN stop) 
{
  if (INPUT.currency == 0xFF)
    return TRUE;
  
  if (type == 0) {
    if (!memcmp(STIS_ACQ_TBL(0).sb_program, gCurInfo[INPUT.currency].name, 3))
      return TRUE;
  }
  else if (type == 1) {
    if (!memcmp(STIS_ACQ_TBL(0).sb_program, gCurInfo[TX_DATA.currency].name, 3))
      return TRUE;
  }
  
  if (stop) {
    DispClrBelowMW(MW_LINE3);
    Disp2x16Msg(GetConstMsg(EDC_IN_CUR_ERROR), MW_LINE3, MW_BIGFONT);
    ErrorDelay();
  }
  return FALSE;
}
...
BOOLEAN InCardTable(void)
{
  ...
  for (card_idx = 0; card_idx < max_card; card_idx++) {
    ...
    if ((memcmp(INPUT.sb_pan, STIS_CARD_TBL(0).sb_pan_range_low, 5) >= 0)  &&
     ...
     //     if (CheckCurrency(0, FALSE))   //15-10-15 JC ++
     //       goto VALID_PAN2;
    //    }
	  if (STIS_ACQ_TBL(0).b_id == STIS_CARD_TBL(0).b_acquirer_id)
    ...
    //  if (card_idx < max_card)   // search next card table //13-07-11 JC ++
    //    continue;
...
BOOLEAN GetAmount(BOOLEAN aTipsNeeded, DWORD aFirstKey)
{
  ...
  //  memcpy(&kbdbuf[1], STIS_TERM_CFG.sb_currency_name, 3);
    memcpy(&kbdbuf[1],gCurInfo[INPUT.currency].name, 3);                               //15-10-15 JC ++
...
//*****************************************************************************
//  Function        : Expire_diff
//  Description     : return true id allow expire date
//  Input           : N/A
//  Return          : TRUE;  // valid input
//                    FALSE; // user cancel
//  Note            : N/A
...
DWORD var_i, var_j;
 ReadRTC(&dtg);
  ...
  DispErrorMsg(GetConstMsg(EDC_IN_CARD_EXPIRED));
   if (APM_WaitKey(KBD_TIMEOUT, 300) == MWKEY_ENTER) {  
   }
	//  return FALSE;
   return TRUE;
